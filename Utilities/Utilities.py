...
def GetUserInput(
    ...
    """
        ...
        Parameters :
            ...
            DefaultValue (used for blank entry) : 
            ...
            DefaultIfWrong : if true, default value is also used for wrong entry (not in possibles, not between min and max, ...)
        ...
        if ValueType.lower() == "int" or ValueType.lower() == "integer":
            ...
            if (MyData.isdigit() or 
                ...
                if PossibleValues is not None:
                    ...
                    elif DefaultValue is not None and DefaultIfWrong:
                ...
                if DefaultValue is not None:
                    # return default value
                    return DefaultValue
                elif PossibleValues is not None:
                    # ask again
                    ErrorMessage = SpecificErrorMessage if SpecificErrorMessage is not None else (f"Merci de rentrer un nombre parmi les suivants : {PossibleValues}.")
            ...
            if MyData != "":
                if PossibleValues is not None:
                    ...
                    if not MyDataToCompare in MyPossibleValues:
                        if DefaultValue is None or not DefaultIfWrong:
                            # no default value specified, so ask again
                ...
                # empty entry
                if PossibleValues is not None and str(DefaultValue).lower() == "random" :
                    # draw an random value in possible values
                    MyData = PossibleValues[random.randint(0, len(PossibleValues)-1)]
                    # return random value
                    return MyData
